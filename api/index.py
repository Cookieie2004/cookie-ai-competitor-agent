...
真实 LLM（千问 Qwen）+ 真实搜索（博查 Bocha）+ 真实抓取 + SQLite 持久化，绝不 demo。
"""
...
app = FastAPI(title="曲奇 Cookie AI 竞品情报工作台 API", version="2.0.0")
...
def llm_ping():
    try:
        ...
        return {"ok": True, "model": settings.qwen_model, "reply": reply}
...
def post_task(body: CreateTaskBody):
    return create_task(body.query, mode=body.mode, model_override=resolve_task_model(body.model))

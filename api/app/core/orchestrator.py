"""曲奇 Cookie 深度调研编排引擎（真实大脑，对标 Deep Research）。
...
模型分配：核心、辅助与快速任务按 .env 中的 Qwen 配置分层路由。
...
_MODEL_OVERRIDE: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "cookie_model_override", default=None
)
...
def _model(tier: str) -> str:
    ...
    if override:
        return override
    if tier == "core":
        return _settings.qwen_model_core
    if tier == "fast":
        return _settings.qwen_model_fast
    return _settings.qwen_model_aux
...
def create_task(
    ...
    clarifications = {"_mode": mode}
    if model_override:
        clarifications["_model_override"] = model_override
    db.save_task(task_id, query, clarifications)
    return {
        "taskId": task_id,
        "needClarify": True,
...
def _clarify_questions(query: str) -> List[Dict[str, Any]]:
    ...
    关键改进（解决「调研 Codex 结果只讲 Codex 不讲竞品」）：
    - 先判定调研对象到底是什么、属于什么领域；
...
使用辅助模型生成稳定 JSON；失败重试一次后再使用正则兜底。
    """
    msgs = [
        {"role": "system", "content": (
        ...
        data = chat_json(
            [
                {"role": "system", "content": (
                    ...
                    "category 要给一个能精准消歧的品类短语（避免品牌名歧义，如 Codex 应识别为「AI编程工具/AI代码编辑器」）。"
...
def _dispatch_experts(query: str, brands: List[str], focus: List[str]) -> Dict[str, Any]:
    ...
    try:
        data = chat_json(
            [
                {"role": "system", "content": (
            "你是曲奇 Cookie 首席研究协调官。从专家名册中为本次调研挑选最合适的团队。"
...
def _sentiment_relevant(brand: str, cat_keywords: List[str], title: str, text: str) -> bool:
    ...
    解决「调研 Codex 抓到美甲」：品牌名命中即相关；若品牌名未命中，
...
async def run_pipeline(task_id: str, sub_id: str = "") -> AsyncIterator[Dict[str, Any]]:
    ...
    _MODEL_OVERRIDE.set(clar.get("_model_override") or None)
    ...
    # 品类关键词（用于消歧 + 相关性过滤，如 Codex→AI编程工具，避免抓到「美甲」等同名内容）
...
def _assemble_report(query, brands, focus, dispatch, claims, evidences, images,
    ...
    glossary = [
        {"term": "交叉验证", "definition": "同一结论由 ≥2 个独立来源支撑，判为高置信。", "source": "Cookie 证据原则"},
        {"term": "无证据不立论", "definition": "任何数据型结论必须挂载 evidence_ids，否则标记待验证。", "source": "Cookie 证据原则"},

...
def _resolve_db_path() -> Path:
    ...
    本地：app/data 下既有 SQLite 文件（持久）。
    Vercel：文件系统只读，唯一可写目录是 /tmp（函数生命周期内有效，
    用户已接受刷新/重启后不持久化）。可用环境变量 COOKIE_DB_PATH 覆盖。
    """
    override = os.environ.get("COOKIE_DB_PATH")
    if override:
        return Path(override)
    if os.environ.get("VERCEL") or os.environ.get("AWS_LAMBDA_FUNCTION_NAME"):
        return Path("/tmp/cookie.db")
    return Path(__file__).resolve().parent.parent / "data" / "cookie.db"

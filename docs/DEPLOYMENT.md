...
本文档说明曲奇 Cookie 的本地运行与 Vercel 云部署，以及 `backend/` 与 `api/` 两份后端代码的同步约定。
...
cp .env.example .env          # 填入 QWEN_API_KEY 等密钥
...
2. 在 **Project Settings → Environment Variables** 配置密钥（**切勿**把 `.env` 提交到仓库）：
   ...
   - 其余可选：`QWEN_MODEL*` / `DOUYIN_COOKIE` / `BILIBILI_COOKIE` / `XHS_COOKIE` / `ENABLE_DEMO_FALLBACK`

"""曲奇 Cookie AI 竞品情报工作台后端入口（FastAPI）。

挂载：48 专家 API + 任务创建/澄清 + SSE 思维流 + 报告/历史 + 仪表盘统计
+ 全局证据溯源库 + 竞品监控订阅 + 专家工作量看板 + 健康/验证接口。
真实 Qwen LLM + 真实搜索（博查 Bocha）+ 真实抓取 + SQLite 持久化。
"""
...
from app.core.config import get_settings, resolve_task_model
...
API_NAME = "曲奇 Cookie AI 竞品情报工作台 API"
STATIC_DIR = Path(__file__).resolve().parent.parent / "static"

app = FastAPI(title=API_NAME, version="2.0.0", docs_url=None)
...
def docs():
    """使用项目内置 Swagger 资源，避免本地访问依赖外部 CDN。"""
...
def llm_ping():
    try:
        ...
        return {"ok": True, "model": settings.qwen_model, "reply": reply.strip()}
...
def post_task(body: CreateTaskBody):
    return create_task(
        body.query,
        mode=body.mode,
        model_override=resolve_task_model(body.model),
    )
